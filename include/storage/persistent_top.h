...
void PersistentTop::list(BackInsertIterator entries_inserter) {
  ...
  std::streamoff off = 0;
  uint32_t entries_count = *reinterpret_cast<uint32_t*>(buf + off);
...
class PersistentTop {
 public:
  explicit PersistentTop(int max_entries_count = 0) :
      max_entries_count_(max_entries_count)
  { }
...
int current_file_index = active_file_index_;
  ...
  for (int read_entries = 0; read_entries < total_entries; ++read_entries) {
    ...
    {
      uint32_t entry_size;
      data_files_[current_file_index].read(reinterpret_cast<char*>(&entry_size),
                                           sizeof(entry_size));
      entry.resize(entry_size);
      data_files_[current_file_index].read(
          reinterpret_cast<char*>(entry.data()), entry_size);
    }
